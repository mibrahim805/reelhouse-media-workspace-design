...
function preferredQuality(qualities: QualityOption[]) {
  return qualities.find((quality) => quality.value === '1080') ?? qualities[0]
}
...
export function QualityDialog({
  ...
  onClose: () => void
  onConfirm: (quality: QualityOption) => void
}) {
  const [selected, setSelected] = useState(
    () => preferredQuality(qualities)?.value ?? 'best',
  )
  ...
  const chosen =
    qualities.find((quality) => quality.value === selected) ?? qualities[0]
      ...
      <div className="animate-in slide-in-from-bottom-4 fade-in relative z-10 w-full max-w-md overflow-hidden rounded-t-2xl border border-border bg-popover shadow-2xl duration-200 sm:rounded-2xl">
        ...
        <div className="flex items-center gap-2 border-t border-border p-3">
          ...
          >
            ...
            Download {chosen?.label ?? 'Best'} · {chosen?.size ?? 'Unknown size'}
          </button>
        </div>
      </div>
    </div>
